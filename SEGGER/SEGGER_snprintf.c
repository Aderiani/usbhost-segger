...
static int _vsnprintf(SEGGER_BUFFER_DESC* pBufferDesc, SEGGER_SNPRINTF_CONTEXT* pContext, const char* sFormat, va_list ParamList) {
  ...
  do {
    ...
    if (c == '%') {
      ...
      case 'o':
        v = va_arg(z, int);
        _PrintUnsigned(pBufferDesc, pContext, v, 8, Flags, Width, Precision);
        break;
      case 'u':
        v = va_arg(z, int);
      ...
      case 's':
        {
          ...
          if (Precision < 0) {
            ...
            Precision = (int)SEGGER_PTR_DISTANCE(t, s);
          }
          if (Width < 0) {
            Width = Precision;
          }
          //
          Len = 0;
          for (t = s; Len < Precision && *t != '\0'; ++t) {
            ++Len;
          }
          //
          if (Flags & SEGGER_PRINTF_FLAG_ADJLEFT) {
            for (i = 0; i < Len; ++i) {
              _StoreChar(pBufferDesc, pContext, *s++);
            }
            while (Len < Width) {
              _StoreChar(pBufferDesc, pContext, ' ');
              ++Len;
      ...
      case 'p':
        {
          const void* p;
                U32   u;
          p = va_arg(z, const void*);
          //
          // On 64-bit systems the upper 32 bits are not printed for now.
          //
          u = (U32)SEGGER_PTR2ADDR(p);
          _PrintUnsigned(pBufferDesc, pContext, u, 16, Flags, -1, 8);
        }
        break;
      default:
Custom:
